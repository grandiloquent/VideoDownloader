...
static int mbedtls_net_connect_timeout(mbedtls_net_context *ctx, const char *host, const char *port,
    ...
    // proto == MBEDTLS_NET_PROTO_UDP ? IPPROTO_UDP :
        ...
        if (connect(ctx->fd, cur->ai_addr, cur->ai_addrlen) == 0) {
            ...
            while (1) {
                ...
                if (ret == -1) {
                    ...
                    slen = sizeof(int);
                    if ((getsockopt(fd, SOL_SOCKET, SO_ERROR, (void *) &opt, &slen) == 0) &&
                        (opt > 0)) {
                        close(fd);
                        ctx->fd = -1;
                        ret = MBEDTLS_ERR_NET_CONNECT_FAILED;
...
static int https_connect(HTTP_INFO *hi, char *host, char *port) {
    ...
    if (https == 1) {
        ...
        ret = mbedtls_ctr_drbg_seed(&hi->tls.ctr_drbg, mbedtls_entropy_func, &hi->tls.entropy, NULL,
                                    0);
        if (ret != 0) {
            return ret;
        }
